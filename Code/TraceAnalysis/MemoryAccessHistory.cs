namespace TraceAnalysis
{
    ...
    public class MemoryAccessHistory
    {
        ...
        /// </summary>
        public bool IsParentChildThreadRel(bool checkHB, MemoryAccess write, MemoryAccess read)
        {
        ...
        public IEnumerable<RacyAccess> GetPotentialRacyAccesses(bool checkHB)
        {
            ...
            foreach (var write in nullToNonNullWrites)
            {
                ...
                while (current != null)
                {
                    ...
                    int gap = (int)Math.Ceiling(1.0 * Math.Max(Constants.ExternalInterferenceLambda, Math.Abs(write.Timestamp - access.Timestamp)) / TimeSpan.TicksPerMillisecond);
                    ...
                    if (((access.ThreadId != write.ThreadId) || (access.TaskId != write.TaskId)) && !IsParentChildThreadRel(checkHB, write, access))
                    {
                        ...
                        */
                        ...
                        //if (access.AccessType == MemoryAccessType.Use && !threadMap.Contains(access.ThreadId))
                        {
                    ...
                    if (IsParentChildThreadRel(checkHB, write, access))
                    {
            ...
            foreach (var d in disposes)
            {
                ...
                while (current != null)
                {
                    ...
                    int gap = (int)Math.Ceiling(1.0 * Math.Max(Constants.ExternalInterferenceLambda, Math.Abs(d.Timestamp - access.Timestamp)) / TimeSpan.TicksPerMillisecond);
                    if (gap > Constants.NearMissThresholdMs)
                    {
                        break;
                    }
                    ...
                    // Check if (1) the two memory accesses happen on different threads, and
                    // (2) the two threads are not in HB relation based on their vector clocks
                    if (((access.ThreadId != d.ThreadId) || (access.TaskId != d.TaskId)) && !IsParentChildThreadRel(checkHB, d, access))
                    {
                        /*
                    ...
                    if (IsParentChildThreadRel(checkHB, d, access) == true)
                    {
                        this.hasHappensBeforeConflicts = true;
                    }
