...
namespace NetMQ.Tests
{
    ...
    {
        private readonly int WAIT_TIME_MS = 300;
        ...
        public void Monitoring()
        {
            ...
            using (var reqMonitor = new NetMQMonitor(req, "inproc://req.inproc", SocketEvents.Connected))
            {
                ...
                Assert.True(listeningEvent.WaitOne(WAIT_TIME_MS));
                Assert.True(connectedEvent.WaitOne(WAIT_TIME_MS));
                Assert.True(acceptedEvent.WaitOne(WAIT_TIME_MS));
            }
        }
        ...
        public void AddSocketDuringWork()
        {
            ...
            using (var poller = new NetMQPoller { router1 })
            {
                ...
                dealer1.SendFrame("1");
                Assert.True(signal1.WaitOne(WAIT_TIME_MS));

                dealer2.SendFrame("2");
                Assert.True(signal2.WaitOne(WAIT_TIME_MS));
        ...
        [Fact(Skip = "Instrumentation breaks test")]
        public void AddSocketAfterRemoving()
        {
            ...
            using (var poller = new NetMQPoller { router1, router2 })
            {
                ...
                Assert.True(signal2.WaitOne(WAIT_TIME_MS));
                dealer3.SendFrame("3");
                Assert.True(signal3.WaitOne(WAIT_TIME_MS));
        ...
        public void AddTwoSocketAfterRemoving()
        {
            ...
            using (var poller = new NetMQPoller { router1, router2 })
            {
                ...
                dealer1.SendFrame("1");
                Assert.True(signal3.WaitOne(WAIT_TIME_MS));
                Assert.True(signal4.WaitOne(WAIT_TIME_MS));
                Assert.True(signal2.WaitOne(WAIT_TIME_MS));
...
*/
        ...
        public void TestPollerDispose()
        {
            ...
            {
                poller.RunAsync();
                Assert.True(signal.WaitOne(WAIT_TIME_MS));
        ...
        public void NativeSocket()
        {
            ...
            using (var socket = new Socket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp))
            {
                ...
                using (var poller = new NetMQPoller())
                {
                    ...
                    //Assert.False(socketSignal.WaitOne(WAIT_TIME_MS));

                    // sending a message back to the socket
                    streamServer.SendMoreFrame(identity).SendFrame("a");
                    Assert.True(socketSignal.WaitOne(WAIT_TIME_MS));
